...
public class QuestionsXmlImport {
    ...
    private void importQuestion(Element questionElement, Course course) {
        ...
        switch (type) {
            ...
            case Question.QuestionTypes.CODE_ORDER_QUESTION:
                ...
                break;
            case Question.QuestionTypes.WORD_ASSIGNMENT_QUESTION:
                questionDetailsDto = importWordAssignmentQuestion(questionElement);
                break;
            default:
    ...
    private QuestionDetailsDto importWordAssignmentQuestion(Element questionElement) {
        List<PhraseDto> phrasesDto = new ArrayList<>();
        for (Element phraseElement : questionElement.getChild("phrases").getChildren("phrase")) {

            String phraseContent = phraseElement.getAttributeValue("content");
            String phraseWord = phraseElement.getAttributeValue("word");

            PhraseDto phraseDto = new PhraseDto();
            phraseDto.setContent(phraseContent);
            phraseDto.setWord(phraseWord);

            phrasesDto.add(phraseDto);
        }
        WordAssignmentQuestionDto wordAssignmentQuestionDto = new WordAssignmentQuestionDto();
        wordAssignmentQuestionDto.setPhrases(phrasesDto);
        return wordAssignmentQuestionDto;
    }
