...
public class XMLQuestionExportVisitor implements Visitor {
    ...
    @Override
    public void visitQuestionDetails(WordAssignmentQuestion question) {
        this.currentElement.setAttribute("type", Question.QuestionTypes.WORD_ASSIGNMENT_QUESTION);

        Element phrasesElement = new Element("phrases");
        this.currentElement.addContent(phrasesElement);

        this.currentElement = phrasesElement;
        question.visitPhrases(this);
    }
    ...
    @Override
    public void visitPhrase(Phrase phrase) {
        Element phraseElement = new Element("phrase");

        phraseElement.setAttribute("content", phrase.getContent());
        phraseElement.setAttribute("word", phrase.getWord());

        this.currentElement.addContent(phraseElement);
    }
